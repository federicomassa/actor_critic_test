...
for epoch in range(num_epochs):
    ...
    for k in range(100):
        ...
        target = r + y*np.max(model.predict(new_s.reshape(-1,3)))
        target_vec = model.predict(s.reshape(-1,3))[0]
        target_vec[a] = target
        # print(target_vec)
        # hist = model.fit(s.reshape(-1,3), target_vec.reshape(-1,3), batch_size = 100,epochs=1, verbose=0)
        #
        # #print(hist.history)
    ...
    # if hist.history['loss'][0] < best_loss:
        # best_loss = hist.history['loss'][0]
    # print("Best loss: ", best_loss, " Total reward: ", total_reward)
    print("Total reward: ", total_reward)
    plt.plot(epoch, total_reward, 'ro')
